...
const iconStyle = computed(() => {
  return {
    backgroundImage: `url("${props.paymentMethod.icon}")`,
  }
})
...
<template>
  ...
  >
    ...
    >
      ...
      >
        <BaseIcon
          v-if="paymentMethod.icon === 'evm'"
          size="54"
          name="other-wallets"
          :class="$style.icon"
        />
        <div
          v-else-if="paymentMethod.icon"
          :class="$style.icon"
          :style="iconStyle"
        />
